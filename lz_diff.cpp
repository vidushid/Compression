...
//************************************************************************************************
vector<uint8_t> write_in_vector(string filename){
    ifstream file(filename, std::ios::binary); // open the file in binary mode
    vector<uint8_t> data; // create a vector to store the data
    char c;
    while (file.get(c)) { // read the file character by character
        data.push_back(static_cast<uint8_t>(dna_code(c))); // store each character in the vector
    }
...
void write_to_file(vector<uint8_t> inp, string fn){
    ...
    for (int i = 0; i < inp.size(); ++i) {
        ...
        //outFile.put(inp[i]); //static_cast<char>
    }
    outFile.close();
}
//*************************************************************************************************
int main(){
    contig_t delta, decom,ref, seq;
    string refer = "ref_lz.txt";
    //TODO: take input of file names
    vector<string> sequences = {"seq_lz.txt"};
    vector<string> outputfile = {"out1.txt"};
    ref=write_in_vector(refer);
    unique_ptr<CLZDiffBase> lz_diff;
    lz_diff = make_unique<CLZDiff_V2>();
    lz_diff->SetMinMatchLen(20);
    lz_diff->Prepare(ref);
    for (int i=0; i<sequences.size(); ++i){
        seq=write_in_vector(sequences[i]);
        for (int j=0; j<10;j++){
            cout << static_cast<int>(seq[j]) << endl;
        }
        lz_diff->Encode(seq,delta);
        cout << delta.size();
        lz_diff->Decode(ref,delta,decom);
        ofstream outFile("encoded.txt");
        for (int i=0; i<delta.size(); ++i){
            outFile.put(delta[i]);
        }
        outFile.close();
        write_to_file(decom,outputfile[i]);
    }
    return 0;
}
