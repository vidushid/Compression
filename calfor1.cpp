...
pair<vector<uint8_t>,size_t> zstd_decompress(uint8_t* com_ptr, size_t ComSize){
    ...
    return make_pair(NumAfterDecompress,DecomSize);
}
...
//************************************************************************************************
void compress(string file_name,unordered_map<string, vector<string>> lift1, 
unordered_map<string, int> ref_index, unordered_map<string, int> seq1_index,
pair<vector<string>, vector<string>> reference, pair<vector<string>, vector<string>> sequence1){
	...
	for(auto i:lift1){
		if(i.second.size()==1){
			...
			vector<string> str2 = split(i.second[0],"_");
...
int main() {
    ...
    vector<string> fa_files={"CHM13Y.fa","HG002.1.fa"};
    vector<string> paf_files={"halign1.paf"};
	vector<string> output_lift={"lift1.txt"};
	...
	for(auto i:lift1){
		lifted[i.first].push_back(i.second[0]);
	}
	cout << "before compress";
	compress("cal1.bin" ,lift1 ,ref_index ,seq1_index, reference,sequence1);
	cout << "after compress";

	ifstream f2("cal1.bin");
	int cstop=0;
	cout << "what";
	for (string line; getline(f2, line); ) {
		cout << "what";
        if (cstop==1){
			break;
		}
		cout << "what";
		contig_t vec(line.begin(), line.end());
		cstop+=1;
		size_t size=line.size();
		pair<contig_t,size_t> zs_de = zstd_decompress(&vec[0],size);
		contig_t one,two,three;
		//uint8_t * th = &three
		one=write_in_vector(reference.second[0].substr(0,50000));
		unique_ptr<CLZDiffBase> lz_diff;
		lz_diff = make_unique<CLZDiff_V2>();
		lz_diff->SetMinMatchLen(20);
		lz_diff->Prepare(one);
		lz_diff->Decode(one,zs_de.first,three);
		cout << three.size();
		write_to_file(three,"de.txt");
